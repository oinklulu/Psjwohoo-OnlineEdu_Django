...
# 不用这个  from django.contrib.auth.mixins import LoginRequiredMixin
...
# 此处多加一个ModeifyPwdView而不是写进ResetView中需要考虑清楚（不懂看视频吧）
class ModifyPwdView(View):
    def post(self, request):
        ...
        # 验证失败说明密码位数或者格式不对。
            ...
            return render(
                request, "password_reset.html", {
                    "email": email, "modifypwd_form": ModifyPwdForm})
...
class UploadImageView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if image_form.is_valid():
            image_form.save()
            # # 取出cleaned data中的值(全部验证通过的都会放入cleaned_data中),一个dict
